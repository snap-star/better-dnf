...
class UpdateSelector:
    ...
    def display_update_summary(cls, packages: List[PackageUpdate]) -> None:
        ...
        for pkg in packages:
            ...
            importance_counts[pkg.importance] = importance_counts.get(pkg.importance, 0) + 1
        ...
        for category in UpdateCategory:
            if category in category_counts:
                ...
                elif category == UpdateCategory.KERNEL:
                    status = "⚠️  Review"
                elif category == UpdateCategory.DRIVER:
                    status = "⚠️  Review"
                else:
                    status = "✅ Safe"
    ...
    def display_packages_by_category(
        ...
        for i, pkg in enumerate(sorted_packages[:max_display]):
            importance_display, importance_color = cls.IMPORTANCE_DISPLAY[pkg.importance]
    ...
    def _select_individual_packages(
        ...
        for pkg in sorted_packages:
            ...
            pre_selected = pkg.importance in (UpdateImportance.CRITICAL, UpdateImportance.HIGH)
    ...
    @classmethod
    def display_package_preview(cls, packages: List[PackageUpdate], max_display: int = 30) -> None:
        """
        ...
        for pkg in sorted_packages[:max_display]:
            importance_display, importance_color = cls.IMPORTANCE_DISPLAY[pkg.importance]
            category_display, category_color = cls.CATEGORY_DISPLAY.get(pkg.category, ("Unknown", "white"))
