...
class DNFParser:
    ...
    @classmethod
    def get_update_info(cls, package_name: Optional[str] = None) -> str:
        """
    ...
    def parse_check_update(cls, output: str) -> List[PackageUpdate]:
        ...
        for line in output.splitlines():
            line = line.strip()
            if not line or line.startswith("Last") or line.startswith("Obsoleting") or line.startswith("Upgrades"):
                continue
    ...
    @classmethod
    def parse_advisory_line(cls, line: str) -> Optional[UpdateAdvisory]:
        """
    ...
    @classmethod
    def categorize_package(cls, package: PackageUpdate, user_installed: Optional[set] = None) -> UpdateCategory:
        """
        ...
        driver_patterns = [
            "nvidia", "nouveau", "mesa", "vulkan", "libgl",
            "wl", "akmod", "kmod", "driver",
            "firmware", "microcode",
            "wifi", "wireless", "bluetooth",
            "sound", "alsa", "pulseaudio", "pipewire",
            "gpu", "graphics", "video",
        ]
        ...
        system_patterns = [
            "glibc", "systemd", "dbus", "polkit", "sudo", "pam",
            "selinux", "firewalld", "networkmanager", "systemd-",
            "grub", "dracut", "plymouth",
        ...
        if package.repository:
            ...
            third_party_repos = [
                "rpmfusion", "google-chrome", "spotify", "docker",
                "copr", "playonlinux", "wine", "steam",
                "pgadmin", "code", "vscode",
            ]
        ...
        for line in security_output.splitlines():
            if line.strip() and not line.startswith('Name'):  # Skip header
                ...
                if len(parts) >= 4:
                    ...
                    package_name = package_full.split('-')[0] if '-' in package_full else package_full
                    ...
                    if len(name_parts) > 1:
                        ...
                        name_without_version = '-'.join(name_parts[:-1]) if name_parts[-1][0].isdigit() else package_full
