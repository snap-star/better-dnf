...
class TestRunCommand:
    ...
    def test_run_command_success(self):
        ...
        with patch(
            "better_dnf.parser.subprocess.run", return_value=mock_result
        ) as mock_run:
    ...
    def test_run_command_timeout(self):
        ...
        with (
            patch(
                "better_dnf.parser.subprocess.run",
                side_effect=subprocess.TimeoutExpired("cmd", 300),
            ),
            pytest.raises(RuntimeError, match="Command timed out"),
        ):
    ...
    def test_run_command_generic_error(self):
        ...
        with (
            patch("better_dnf.parser.subprocess.run", side_effect=OSError("boom")),
            pytest.raises(RuntimeError, match="Failed to run command"),
        ):
            DNFParser.run_command("echo hello")
...
class TestCommandHelpers:
    ...
    def test_get_check_update_output_failure(self):
        ...
        with (
            patch.object(DNFParser, "run_command", return_value=("err\n", 1)),
    ...
    def test_get_update_info_no_package(self):
        ...
        with patch.object(
            DNFParser, "run_command", return_value=("adv\n", 0)
        ) as mock_run:
    ...
    def test_get_security_updates_success(self):
        ...
        with patch.object(
            DNFParser, "run_command", return_value=("sec\n", 0)
        ) as mock_run:
...
class TestGetInstalledVersion:
    ...
    def test_installed(self):
        ...
        with patch.object(
            DNFParser, "run_command", return_value=("153.0.1\n", 0)
        ) as mock_run:
...
class TestParseAdvisoryLineVariants:
    ...
    def test_multiple_cves(self):
        ...
        line = (
            "FEDORA-2024-abc444 security/critical CVE-2024-1111 CVE-2024-2222 openssl"
        )
        advisory = DNFParser.parse_advisory_line(line)
...
class TestParseAll:
    ...
    def _patch_parse_all(
        self, user_installed=None, security="", sizes=None, installed=None
    ):
        return [
            patch.object(
                DNFParser,
                "get_user_installed_packages",
                return_value=user_installed or set(),
            ),
    ...
    def test_parse_all_fetch_sizes_false(self):
        ...
        output = "firefox.x86_64 153.0.2-1.fc44 updates\n"
        with (
            patch.object(DNFParser, "get_user_installed_packages", return_value=set()),
...
class TestFetchDownloadSizes:
    ...
    def test_fetches_and_updates_in_place(self):
        ...
        with patch.object(
            DNFParser, "get_download_sizes", return_value={"firefox": 999}
        ) as mock_get:
